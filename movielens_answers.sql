...
-- Question 5: Find the total number of movies in each genre: list the results in ascending numerical orders
select genres.name, count(genres_movies.genre_id) from genres 
	inner join genres_movies on genres.id = genres_movies.genre_id 
	group by genres_movies.genre_id order by count(genres_movies.genre_id) asc;